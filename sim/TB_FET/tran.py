...
def main(idk):
    return
    # find the raw file in the current simulation directory
    print(Path("."))
    raw_files = list(Path(".").glob("*.raw"))

    if not raw_files:
        raise RuntimeError("No raw file found")
    ...
    dfs["v(vds)"] = dfs["v(vdd)"] - dfs["v(ibias)"]
    
    plt.figure()
    plt.plot(df["i(v.xdut.v2)"], df["v(vds)"])
    plt.xlabel("Id [A]")
    plt.ylabel("Vds [V]")
    plt.show()
